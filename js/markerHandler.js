...
AFRAME.registerComponent("markerhandler", {
  ...
  handleOrderSummary: async function () {
    ...
    td3.setAttribute("class", "no-line text-cente");
  ...
  handleRatings: async function (toy) {
    ...
    if(currentOrders.length > 0 && currentOrders == toy.id) {
      ...
      var saveSubmitButton = document.getElementById("save-rating-button")
      saveSubmitButton.addEventListener("click", ()=>{
        ...
        firebase.firestore().collection("toys").doc(toy.id).update({
          last_rating: ratingInput,
          last_review: feedbackInput
        }).then(()=>{
          swal({
            icon: "success",
            title: "Thanks A Lot",
            text: "Thanks for Rating! Hope you enjoyed",
            buttons: false,
            timer: 5000,
          })
        })
      ...
      swal({
        icon: "warning",
        title: "No Toy has been ordered",
        text: "Order a Toy to review it",
        buttons: false,
        timer: 5000,
      })
    }
